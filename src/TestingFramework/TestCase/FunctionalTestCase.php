...
abstract class FunctionalTestCase extends AbstractTestCase
{
    ...
    protected function setUpFrontendRootPage($pageId, array $typoScriptFiles = [], array $sites = [])
    {
        ...
        $this->setUpSites($pageId, $sites);
    }

    /**
     * @param int $pageId
     */
    protected function setUpPageRecord($pageId)
    {
     ...
     * @param array $typoScriptFiles
     */
    protected function setUpTemplateRecord($pageId, array $typoScriptFiles)
    {
        $templateFields = [
            'pid' => $pageId,
            'title' => '',
    ...
    protected function getFrontendResponse($pageId, $languageId = 0, $backendUserId = 0, $workspaceId = 0, $failOnFailure = true, $frontendUserId = 0)
    {
        ...
        if (class_exists('SebastianBergmann\\Template\\Template')) {
            $template = new Template('ntf://Frontend/Request.tpl');
        } else {
